 ...
 * Supports locale-specific reports and caching for better performance.
 */
@Slf4j
@Component
...
public class BirtReportLoader {
    ...
    private static final String DEFAULT_REPORTS_DIR = System.getProperty("user.home") 
            + File.separator + ".mifosx" + File.separator + "birtReports" + File.separator;
    ...
    public IReportRunnable loadReport(String reportName, java.util.Locale locale) {
        ...
        log.info("Loading BIRT report: {} (locale: {}) from path: {}", 
                reportName, locale != null ? locale.getLanguage() : "en", reportPath);
        ...
        if (!reportFile.canRead()) {
            ...
            throw reportErrorHandler.reportError(
                "error.msg.reporting.report.not.readable",
                "Report file is not readable: " + reportName);
        }
        ...
        try {
            ...
            throw reportErrorHandler.reportError(
                ...
                "Failed to load report design: " + reportName, e);
        }
    }
    ...
    /**
     * Returns the base directory for BIRT reports.
     * Priority: Configured path → Default directory
     */
