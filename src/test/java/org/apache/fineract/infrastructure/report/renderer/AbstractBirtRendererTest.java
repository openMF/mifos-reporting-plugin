...
class AbstractBirtRendererTest {
  ...
  @Mock private IReportDocument reportDocument;
  @Mock private IRenderTask renderTask;
  ...
  void shouldCloseDocumentIfTaskCreationFails() throws Exception {
    ...
    doThrow(new RuntimeException("BIRT Engine Error"))
        .when(reportEngine)
        .createRenderTask(reportDocument);

    assertThrows(
        RuntimeException.class,
        () -> renderer.render(reportEngine, "/dummy/path.rptdocument", "TestReport"));
